...
#Import Breweries.csv file into Breweries dataframe - CHANGE FOR GitHub
...
BreweryCount <- data.frame(sort(table(Breweries$StateName), decreasing=TRUE))
#assign meaningful column name to Brewery Count for output
colnames(BreweryCount)<- c("State", "NumberOfBreweries")
...
BreweryCount$region <- tolower(BreweryCount$State)
BreweryCountGeo <- merge(states, BreweryCount, by="region")
qplot(long, lat, data=BreweryCountGeo, geom="polygon", fill=NumberOfBreweries, group=group)
...
snames <- merge(snames, BreweryCount, by="region")
# And finally, to put everything together: 
map1 <- ggplot(BreweryCountGeo, aes(long, lat)) + geom_polygon(aes(group=group, fill=NumberOfBreweries)) + geom_text(data=snames, aes(long, lat, label=NumberOfBreweries))
map1
#End of first heat map

map2 <- map1 + scale_fill_continuous(low="lightskyblue", high="royalblue", guide="colorbar") + theme_bw() + labs(title = "Number of Breweries in Each State", fill = "Count", x="", y="")
map2

#map3 <- map2 + scale_linetype_continuous(breaks=c()) + scale_x_continuous(breaks = c()) + theme (panel.border = element_blank())
...
ABVMedians <- aggregate(BeerBreweries$ABV ~ BeerBreweries$StateName, data=BeerBreweries, median)
IBUMedians <- aggregate(BeerBreweries$IBU ~ BeerBreweries$StateName, data=BeerBreweries, median)
...
barplot(IBUABV$MedianABVPC, 
        main = "Median Beer Alcohol By Volume by State",
        horiz = TRUE, 
        beside = TRUE,
        xlab = "Median ABV", 
        ylab = "States",
...
IBUABV<-IBUABV[,2:length(IBUABV )]

IBUABV.long<-melt(IBUABV,id.vars="StateName")

ggplot(IBUABV,aes(x=IBUABV$StateName,y=value,fill=factor(gender)))+
